...
from keras.layers import Conv2D, MaxPool2D, Flatten, Dense, Input, Subtract, Lambda, Conv1D, MaxPool1D, MaxPooling2D, GlobalAveragePooling2D
...
class SiameseNetwork:
    ...
    def __init__(self, dataset_path,  learning_rate, batch_size, use_augmentation,
        ...
        if use_CNN:
            ...
            self.input_shape = ((768,))  # Size of sentences vector 长度是经过bert转化后的768
        self.model = []
    ...
    def _construct_siamese_architecture(self, learning_rate_multipliers,
        ...
        convolutional_net.add(Dense(64, activation='sigmoid',
                  kernel_regularizer=l2(
...
# Train loop!!!!start
        for iteration in range(number_of_iterations):
            ...
            # train set
            sentences, labels = self.omniglot_loader.get_train_batch()
            # print(sentences[0].shape)
            # print(labels.shape)
            # print(sentences[0][0].shape)
            ...
            # 每训练evaluate_each次数，就在validation集上做一次验证，并存储loss和acc
            # Each 1 iterations perform a one_shot_task and write to tensorboard the
            ...
            if (iteration + 1) % evaluate_each == 0:
                ...
                validation_accuracyss = self.omniglot_loader.one_shot_test(
                    self.model, support_set_size, number_of_runs_per_alphabet, is_validation=False)
                print('vaaaaaaaaaaaaa', validation_accuracyss)
